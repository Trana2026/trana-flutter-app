...
class EkycDetectionService {
  ...
  /// NCP CLOVA eyed 라이선스 검증. SDK 감지 활성화할려면 필요
  ...
  /// Native 측이 즉시 반환하므로 _pendingFrames는 MethodChannel 마샬링 시간만큼만 증가
  /// [sensorOrientation]: 카메라 센서 방향(후면 90 / 전면 270). Native가 회전 보정에 사용
  Future<void> analyzeFrame(
    ...
    try {
      ...
      if (Platform.isIOS) {
        // iOS: BGRA8888 단일 plane이므로 변환 없이 그대로 전달 (bytesPerRow 동봉)
        ...
        args = {
          ...
          'flip': isFrontCamera,
          // iOS 카메라 플러그인은 프레임을 이미 화면 방향(portrait)으로 회전해 전달하므로 추가 회전 0
          'sensorOrientation': 0,
        };
      } else {
