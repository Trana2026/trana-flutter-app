...
/// 로컬 PDF 생성 + 가짜 S3 저장으로 백엔드 없이 테스트하는 Fake 구현체
class FakePdfRepositoryImpl implements PdfRepository {
  ...
  Future<Result<Uint8List>> generatePdf(PdfEntity entries) async {
    try {
      ...
      var cursor = _PdfCursor(page: currentPage, y: y);
      ...
      debugPrint('[FakePdfRepo] generatePdf unexpected: $e');
  ...
  Future<Result<String>> savePdf(Uint8List pdfBytes) async {
    final key =
        'fake/pdfs/contract_${DateTime.now().millisecondsSinceEpoch}.pdf';
    return Success(key);
  }

  Future<_PdfCursor> _drawSection({
    ...
    return _PdfCursor(page: lastPage, y: nextY);
  }
}

class _PdfCursor {
  const _PdfCursor({required this.page, required this.y});
