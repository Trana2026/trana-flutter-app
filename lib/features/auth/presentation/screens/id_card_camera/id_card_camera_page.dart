...
import 'package:trana/features/ekyc/domain/entities/detection_event.dart';
import 'package:trana/features/ekyc/presentation/viewmodels/adult_kyc_view_model.dart';

/// 신분증 촬영 화면. SDK 자동 감지 + Flutter CameraController 프리뷰
class IdCardCameraPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // SDK 크롭 결과 or takePicture 캡처 후 정지 화면 경로
      ...
      Future<void> init() async {
        try {
          ...
          final backs = cameras
              ...
              .toList();
          // iOS: 메인 광각(backs[0])은 가까운 신분증 초점 불가(최소 초점거리 한계)
          // 초광각(backs[2]=video:5)으로 근거리에서 자동 초점 맞출수있게 함. 안드로이드는 기존 광각 유지
          ...
          sub = service!.idCardStream.listen((event) {
            ...
            switch (event) {
              ...
              case IdCardDetecting():
                ...
                // detecting이 ~15fps로 계속 오기 때문에 매번 cancel/restart하면 타이머가 영원히 발화 안되기 때문
          ...
          // EventChannel onListen 후 감지 시작 (idCardEventSink 보장된 상태)
...
Future<void> _uploadCroppedBytes(
  ...
  try {
    ...
    // 크롭 바이트를 임시 파일로 저장. 정지 화면과 OCR 입력으로 사용
    final tempFile = File(
    ...
    switch (result) {
      ...
      case Failure(:final failure):
        capturedImagePath.value = null; // 오류 시 라이브 프리뷰 복귀
        _handleUploadError(
          context,
          ref,
          cam,
...
/// takePicture 기반 수동 촬영 후 OCR 업로드 (SDK 크롭 변환 실패 시 폴백 포함)
...
/// 업로드 실패 공통 처리. 세션 만료는 스낵바 출력 후 화면 이동, 그 외는 인프레임 빨간 테두리
